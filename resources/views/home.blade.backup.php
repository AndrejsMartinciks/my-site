...
<head>
  ...
  <title>{{ $siteSettings->seo_title ?? 'Clean Source AB | Hemstädning i Stockholm' }}</title>
  <meta name="description" content="{{ $siteSettings->seo_description ?? 'Professionell hemstädning, flyttstädning, byggstädning, fönsterputsning och storstädning i Stockholm.' }}" />
...
<body>
  ...
  <header class="site-header" id="top">
    <div class="container nav-wrap">
      <a class="brand" href="#top" aria-label="CleanS AB startsida">
        <img src="{{ asset('images/logo.png') }}" alt="CleanS AB" class="brand-logo">
        <span>
          <strong>{{ $siteSettings->company_name ?? 'Clean Source AB' }}</strong>
  ...
  <main id="main">
    <section class="hero section">
      <div class="container hero-grid">
        <div class="hero-copy">
          <span class="eyebrow">{{ $siteSettings->hero_eyebrow ?? 'Hemstädning • Flyttstädning • Fönsterputs' }}</span>
<h1>{{ $siteSettings->hero_title ?? 'Rent hemma, mer tid över för livet.' }}</h1>
<p class="lead">{{ $siteSettings->hero_text ?? 'Clean Source AB hjälper hushåll i Stockholm med noggrann städning, tydliga priser och snabb återkoppling. Vi arbetar med professionella rutiner, försäkring och fokus på trygg service.' }}</p>
          <div class="hero-actions">
            <a href="#calculator" class="btn btn-primary">{{ $siteSettings->hero_primary_button_text ?? 'Få kostnadsfri offert' }}</a>
<a href="#services" class="btn btn-secondary">{{ $siteSettings->hero_secondary_button_text ?? 'Se våra tjänster' }}</a>
          </div>
...
@include('partials.calculator')
      ...
      <div class="container">
        ...
        <div class="service-grid">
          ...
          <article class="service-card">
            ...
            <p>Rena fönster med ett jämnt och hållbart resultat för bostäder och mindre lokaler.</p>
            <ul>
              <li>Invändigt och utvändigt</li>
              <li>Erfaret team</li>
              <li>Bättre ljusinsläpp direkt</li>
            </ul>
          </article>
          <article class="service-card">
            <h3>Byggstädning</h3>
            <p>Efter renovering eller byggprojekt ser vi till att damm, rester och smuts försvinner.</p>
            <ul>
              <li>Finputs före inflytt</li>
              <li>Dammreducering</li>
              <li>Säker arbetsmetod</li>
            </ul>
          </article>
          <article class="service-card">
            <h3>Flytthjälp &amp; packning</h3>
            <p>Vi kan kombinera städning med praktisk hjälp före eller efter flytten för en smidigare process.</p>
            <ul>
              <li>Packhjälp</li>
              <li>Bärhjälp</li>
              <li>Samordning med städning</li>
            </ul>
          </article>
        </div>
      </div>
      ...
      <div class="container contact-grid">
        <div>
          ...
          <div class="contact-cards">
            <article>
              <strong>Mobil</strong>
              <a href="tel:{{ preg_replace('/\D+/', '', $siteSettings->phone_primary ?? '') }}">{{ $siteSettings->phone_primary ?? '070 741 37 72' }}</a>
            </article>
            <article>
              <strong>Växel</strong>
              <a href="tel:{{ preg_replace('/\D+/', '', $siteSettings->phone_secondary ?? '') }}">{{ $siteSettings->phone_secondary ?? '08-838-538' }}</a>
            </article>
            <article>
              <strong>E-post</strong>
              <a href="mailto:{{ $siteSettings->email ?? 'info@cleansource.se' }}">{{ $siteSettings->email ?? 'info@cleansource.se' }}</a>
            </article>
            <article>
              <strong>Adress</strong>
              <p>{{ $siteSettings->address ?? 'Ångermannagatan 121' }}, {{ $siteSettings->postal_code ?? '162 64' }} {{ $siteSettings->city ?? 'Vällingby' }}</p>
            </article>
          </div>
        </div>
  ...
  <ul>
    ...
    <div class="container footer-bottom">
      <small>© <span id="year"></span> CleanS AB. Alla rättigheter förbehållna.</small>
    </div>
  </footer>
